...
def cofi_cost_func(X, W, b, Y, R, lambda_):
    ...
    J = 0
    ### START CODE HERE ###  
    for i in range(nm):  # Loop over movies
        for j in range(nu):  # Loop over users
            if R[i, j] == 1:  # Consider only rated movies
                prediction = np.dot(X[i, :], W[j, :]) + b[0, j]  # Predicted rating
                error = prediction - Y[i, j]  # Error term
                J += error ** 2  # Squared error contribution

    J /= 2  # Compute final cost
